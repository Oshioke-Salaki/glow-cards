...
access(all) contract GlowCardsV2 {

    // --- Events ---
    access(all) event GiftCreated(id: UInt64, sender: Address, initialAmount: UFix64)
    ...
    access(all) resource Gift: GiftPublic {
        ...
        init(id: UInt64, sender: Address, payment: @{FungibleToken.Vault}) {
            self.id = id
            self.sender = sender
        ...
        access(all) fun claim(recipient: Address): @{FungibleToken.Vault} {
            ...
            let targetYield = (principalBal * GlowCardsV2.apr) * (secondsElapsed / secondsInYear)
            
            let payoutVault <- self.vault.withdraw(amount: principalBal)
            
            // Withdraw yield from the global treasury if available
            var actualYield = 0.0
            if targetYield > 0.0 && GlowCardsV2.yieldTreasury.balance >= targetYield {
                let interest <- GlowCardsV2.withdrawFromTreasury(amount: targetYield)
    ...
    access(all) resource EscrowManager {
        ...
        access(all) fun createGift(sender: Address, payment: @{FungibleToken.Vault}): UInt64 {
            let id = GlowCardsV2.totalGiftsCreated
            GlowCardsV2.totalGiftsCreated = GlowCardsV2.totalGiftsCreated + 1
            
            let amt = payment.balance
            let newGift <- create Gift(id: id, sender: sender, payment: <-payment)
            self.gifts[id] <-! newGift
            
            emit GiftCreated(id: id, sender: sender, initialAmount: amt)
            return id
        }
        ...
        access(all) fun calculateYield(id: UInt64): UFix64 {
            ...
            let giftRef = (&self.gifts[id] as &GlowCardsV2.Gift?)!
            
            let secondsElapsed = getCurrentBlock().timestamp - giftRef.createdAt
            let secondsInYear = 31536000.0
            return (giftRef.initialAmount * GlowCardsV2.apr) * (secondsElapsed / secondsInYear)
        }
    }
    ...
    access(all) resource Admin {
        access(all) fun fundTreasury(funds: @{FungibleToken.Vault}) {
            ...
            GlowCardsV2.yieldTreasury.deposit(from: <-funds)
    ...
    init() {
        ...
        self.account.storage.save(<-create EscrowManager(), to: /storage/GlowCardsV2EscrowManager)
        let escrowCap = self.account.capabilities.storage.issue<&EscrowManager>(/storage/GlowCardsV2EscrowManager)
        self.account.capabilities.publish(escrowCap, at: /public/GlowCardsV2EscrowManager)
        
        self.account.storage.save(<-create Admin(), to: /storage/GlowCardsV2Admin)
    }
}
